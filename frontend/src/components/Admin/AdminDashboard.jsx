...
const AdminDashboard = () => {
  ...
  return (
    ...
    >
      ...
      {/* Sidebar */}
      <aside className={`bg-[#0f172a] text-white flex flex-col fixed left-0 top-0 z-40 transform transition-all duration-300 ease-in-out h-full ${
        sidebarOpen 
          ? 'w-64' 
          : 'w-16'
      } ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        {/* Logo */}
        <div className={`px-6 py-5 flex items-center gap-3 border-b border-slate-800 transition-all duration-300 ${
          sidebarOpen ? 'justify-start' : 'justify-center px-4'
        }`}>
          <div className={`bg-gradient-to-br from-primary to-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg transition-all duration-300 ${
          ...
          {sidebarOpen && (
            <div className="overflow-hidden">
              <span className="text-xl font-bold tracking-tight text-white">PlaceIQ</span>
          ...
          <div className={`flex ${sidebarOpen ? 'justify-between' : 'justify-center'}`}>
            {sidebarOpen && (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2.5 text-slate-400 hover:text-red-400 hover:bg-red-950/30 rounded-lg text-sm font-medium transition-all duration-150 flex-1 mr-2"
              >
            ...
            <button
              ...
              className={`p-2 text-slate-400 hover:text-slate-200 hover:bg-slate-800/60 rounded-lg transition-all duration-150 ${
                sidebarOpen ? '' : 'mx-auto'
              }`}
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${
        ...
        {/* Top Header Bar */}
        <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b border-gray-200 px-4 md:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5 pl-3 border-l border-gray-200">
              <div className="text-right">
                <p className="text-base font-semibold text-gray-800 leading-tight">{user?.name || 'Admin'}</p>
                <p className="text-xs text-gray-400 capitalize">{user?.role || 'admin'}</p>
              </div>
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-xs font-bold text-white shadow">
                {initials}
              </div>
            </div>
          </div>
