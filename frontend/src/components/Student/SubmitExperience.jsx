...
const SubmitExperience = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let msg = resp?.message || err.message || 'Submission failed';
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <div className="animate-fade-in-up">
            <h3 className="text-xl font-bold text-slate-800 mb-6">Basic Details</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#6d5dfc] focus:border-transparent outline-none transition-all"
                  required
                />
              </div>
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700">Role Applied *</label>
                <input
                  type="text"
                  name="role_applied"
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#6d5dfc] focus:border-transparent outline-none transition-all bg-white"
                >
              ...
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700">CTC Offered (LPA)</label>
                <input
                  type="number"
                  name="ctc_offered"
            ...
            <div className="mt-6">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input
                  ...
                  onChange={handleChange}
                  className="w-5 h-5 text-[#6d5dfc] rounded border-slate-300 focus:ring-[#6d5dfc]"
                />
                <span className="text-slate-700 group-hover:text-[#6d5dfc] transition-colors">Submit Anonymously</span>
              </label>
            </div>
          </div>
        );
      ...
      case 2:
        return (
          <div className="animate-fade-in-up">
            <h3 className="text-xl font-bold text-slate-800 mb-6">Interview Rounds</h3>
            <div className="space-y-6">
              {formData.rounds.map((round, rIndex) => (
                <div key={rIndex} className="bg-slate-50 border border-slate-200 rounded-xl p-6 relative group hover:shadow-md transition-all">
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-bold text-slate-800 bg-white px-3 py-1 rounded-full text-sm border border-slate-200">
                  ...
                  <div className="bg-white p-4 rounded-lg border border-slate-200">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Questions asked</h4>
                        ...
                        <div key={qIndex} className="flex gap-3 items-start animate-fade-in">
                          <div className="flex-1">
                            <input
                              ...
                              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-[#6d5dfc] focus:border-transparent outline-none transition-all text-sm"
                            />
                          </div>
                          <div className="w-32">
                            <select
                              ...
                              className="w-full px-2 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-[#6d5dfc] focus:border-[#6d5dfc]"
                            >
                              <option value="easy">Easy</option>
                              <option value="medium">Medium</option>
                              <option value="hard">Hard</option>
                    ...
                    <button
                      ...
                      className="mt-3 text-sm text-[#6d5dfc] font-medium hover:text-[#5b47d6] flex items-center gap-1 transition-colors"
                    >
            ...
            <div className="flex justify-center mt-8">
              <button
                onClick={addRound}
                className="flex items-center gap-2 px-6 py-3 border-2 border-dashed border-[#6d5dfc] text-[#6d5dfc] rounded-xl hover:bg-[#f3f0ff] transition-all font-medium"
              >
                <span className="text-xl leading-none">+</span> Add Round
              </button>
            </div>
            {formData.rounds.length === 0 && (
              <p className="text-center text-slate-400 italic mt-4">No rounds added yet. Add a round to continue.</p>
            )}
          </div>
        );
      ...
      case 3:
        return (
          <div className="animate-fade-in-up">
            <h3 className="text-xl font-bold text-slate-800 mb-6">Overall Feedback</h3>
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700">Your Overall Experience</label>
                <textarea
                  name="overall_feedback"
                  value={formData.overall_feedback}
                  onChange={handleChange}
                  className="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-[#6d5dfc] focus:border-transparent outline-none transition-all resize-none shadow-sm"
                  rows="6"
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-slate-700">Overall Difficulty</label>
                  ...
                  {['easy', 'medium', 'hard'].map(level => (
                    <label key={level} className={`
                      ...
                      ${formData.overall_difficulty === level
                        ? 'border-[#6d5dfc] bg-[#f3f0ff] text-[#6d5dfc]'
  ...
  if (success) {
    return (
      <div className="max-w-xl mx-auto mt-10 p-8 bg-white rounded-2xl shadow-xl text-center">
        ...
        <p className="text-slate-600 mb-8">Thank you for sharing your experience. It will be live after admin approval.</p>
        <button
          onClick={() => window.location.reload()}
          className="px-8 py-3 bg-[#6d5dfc] text-white rounded-xl font-semibold hover:bg-[#5b47d6] transition-all shadow-lg hover:shadow-[#d7d2ff]"
        >
          Submit Another
        </button>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto my-8 p-8 bg-white rounded-2xl shadow-xl font-sans">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-bold text-slate-900 mb-2">Submit Experience</h2>
        <p className="text-slate-500">Share your interview journey to help juniors prepare better</p>
      </div>
      ...
      <div className="relative flex justify-between mb-12 max-w-2xl mx-auto">
        ...
        <div
          className="absolute top-1/2 left-0 h-1 bg-[#6d5dfc] -translate-y-1/2 z-0 transition-all duration-500"
        ...
        {[1, 2, 3].map((s) => (
          <div key={s} className="relative z-10 flex flex-col items-center">
            <div className={`
              ...
              ${step >= s
                ? 'bg-[#6d5dfc] border-white text-white shadow-lg'
            ...
            <div className={`
              ...
              ${step >= s ? 'text-[#6d5dfc]' : 'text-slate-400'}
            `}>
      ...
      <div className="flex justify-between mt-12 pt-6 border-t border-slate-200">
        {step > 1 ? (
          <button
            ...
            className="px-6 py-3 text-slate-600 font-semibold hover:text-slate-900 hover:bg-slate-50 rounded-xl transition-all"
          >
            Back
          </button>
        ) : <div></div>}
        ...
        {step < 3 ? (
          <button
            onClick={() => setStep(s => s + 1)}
            className="px-8 py-3 bg-[#6d5dfc] text-white rounded-xl font-semibold hover:bg-[#5b47d6] transition-all shadow-lg hover:shadow-[#d7d2ff] hover:-translate-y-0.5"
          >
            Next Step
          </button>
        ) : (
          <button
            ...
            disabled={loading}
            className={`
              px-8 py-3 bg-[#6d5dfc] text-white rounded-xl font-semibold transition-all shadow-lg hover:shadow-[#d7d2ff] hover:-translate-y-0.5
              ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#5b47d6]'}
            `}
          >
